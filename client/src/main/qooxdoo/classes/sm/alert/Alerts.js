 ...
 * Alert notifications.
 *
 * @asset(sm/icons/32/information.png)
 * @asset(sm/icons/32/exclamation.png)
 * @asset(sm/icons/32/error.png)
 */
qx.Class.define("sm.alert.Alerts", {
    type: "static",
    ...
    statics: {
        ...
        warning: function (message, callback, context) {
            (new sm.dialog.Message({
                ...
                "callback": callback || null,
                "context": context || null,
                "image": "sm/icons/32/exclamation.png"
            })).open();
        },
        ...
        errorPopup: function (message, options) {
            ...
            if (options["icon"] === undefined) {
                options["icon"] = "sm/icons/32/error.png";
            }
            if (options["showTime"] == null) {
                options["showTime"] = Number.MAX_VALUE;
            }
            sm.alert.Alerts.infoPopup(message, options);
        },
        ...
        closeAllPopups: function () {
            var info = sm.alert.Alerts.INFO_POPUP;
        ...
        infoPopup: function (message, options) {
            ...
            var info = sm.alert.Alerts.INFO_POPUP;
            if (!info) {
                info = sm.alert.Alerts.INFO_POPUP =
                    new qx.ui.container.Composite(new qx.ui.layout.VBox(4)
                    .set({alignX: "center"}));
                info.getContentElement().addClass("sm-app-popup");
            ...
            if (options["icon"] === false) {
                ...
                options["icon"] = (options["icon"] != null) ? options["icon"] : "sm/icons/32/information.png";
            }
