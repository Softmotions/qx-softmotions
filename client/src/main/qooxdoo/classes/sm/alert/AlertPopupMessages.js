 ...
 */
qx.Class.define("sm.alert.AlertPopupMessages", {
    ...
    members: {
        ...
        activate: function (isNotification) {
            ...
            if (!isNotification) {
                ...
                this.__msgs = null;
                var el = sm.alert.Alerts.infoPopup(buf.join("<br>"), {
                    showTime: hasErrors ? 0 : 5000, // todo review?
                    icon: hasErrors ? "sm/icons/32/exclamation.png" : null
                });
                if (el) { // No errors
                    el.addListenerOnce("disappear", function () {
                        if (sm.alert.Alerts.INFO_POPUP && sm.alert.Alerts.INFO_POPUP.isEmpty()) {
