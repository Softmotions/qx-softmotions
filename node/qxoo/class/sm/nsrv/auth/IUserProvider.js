...
qx.Interface.define("sm.nsrv.auth.IUserProvider", {
    members :
    {
        ...
        /**
         * Получение авторизационных данных пользователя. Необходим для работы алгоритма с Digest авторизацией.
         * Если пользователь с указанным именем найден, во второй аргумент обработчика передаётся объект-пользователь, иначе <code>null</code>
         * Замечание! возможны два варинта выдачи параметров авторизации:
         *  - если доступен пароль пользователя в незашифрованном виде, то объект-пользователь содержит в поле <code>plainPassword<code> его значение,
         *    при этом, поле <code>realmName</code> должно быть <code>undefined</code>
         *  - если не доступен пароль в незашифрованном виде, то объект-пользователь должен содержать поля <code>realmName</code> и <code>digestPassword</code>,
